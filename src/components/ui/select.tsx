...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between whitespace-nowrap rounded-xl border border-[#d8c5af] bg-white px-3.5 py-2 text-sm text-[#4a1c14] font-medium shadow-sm transition-all focus:outline-none focus:ring-2 focus:ring-[#8b2500]/20 focus:border-[#8b2500] disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
      className,
    )}
    {...props}
  >
    ...
    <SelectPrimitive.Icon asChild>
      <ChevronDown className="h-4 w-4 text-[#7c533f] opacity-70" />
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-pointer select-none items-center rounded-xl py-2 pl-3 pr-8 text-sm font-semibold text-[#4a1c14] outline-none transition-colors hover:bg-[#faf4eb] focus:bg-[#faf4eb] focus:text-[#8b2500] data-[disabled]:pointer-events-none data-[disabled]:opacity-50 data-[state=checked]:font-bold data-[state=checked]:text-[#8b2500] data-[state=checked]:bg-[#faf4eb]",
      className,
    )}
    {...props}
  >
    <span className="absolute right-2.5 flex h-4 w-4 items-center justify-center text-[#8b2500]">
